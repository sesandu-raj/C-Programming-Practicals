...
int main (void) {
    ...
    // Create an array to hold the uppercase string
    ...
    for(int i = 0; i < strlen(text); i++) {
        if(text[i] >= 'a' && text[i] <= 'z') {
            upper_text[i] = text[i] - 32;  // Convert to uppercase
        } else {
            upper_text[i] = text[i]; // Copy as is
        }
    }
    upper_text[strlen(text)] = '\0'; // Null-terminate

    printf("After : %s\n", upper_text);

    for (int i = 0; i < strlen(upper_text); i++) // Or you can use "for(int i = 0; array[i] != '\0'; i++)"
    {
